								...
								<tbody>
									...
									<tr>
										...
										<td>${memberInfo.address} ${memberInfo.address2}<br>${memberInfo.address3}
											...
											value="${memberInfo.address2}"> <input
											class="address3_input" type="hidden"
											value="${memberInfo.address3}">
										</td>
									</tr>
						...
						<c:forEach items="${orderList}" var="ol">
							<tr>
								...
								<td class="goods_table_price_td"><fmt:formatNumber
										value="${ol.real_price}" pattern="#,### 원" /> | 수량
									${ol.sales}개 <br> <fmt:formatNumber
										value="${ol.total_sales}" pattern="#,### 원" /> <input
									type="hidden" class="individual_sell_price_input"
									value="${ol.sell_price}"> <input type="hidden"
									class="individual_real_price_input" value="${ol.real_price}">
									<input type="hidden" class="individual_sales_input"
									value="${ol.sales}"> <input type="hidden"
									class="individual_total_sales_input"
									value="${ol.real_price * ol.sales}"> <input
									type="hidden" class="individual_product_no_input"
									value="${ol.product_no}"></td>
							</tr>
						</c:forEach>
...
<form class="order_form" action="/order" method="post">
	...
	<!-- 주소록 & 받는이-->
	<input name="addressee" type="hidden"> <input name="address"
		type="hidden"> <input name="address2" type="hidden"> <input
		name="address3" type="hidden">
		...
		$(".goods_table_price_td").each(
				function(index, element) {
					...
					totalPrice += parseInt($(element).find(
							".individual_total_sales_input").val());
					// 총 갯수
					totalCount += parseInt($(element).find(
							".individual_sales_input").val());
					// 총 종류
					totalKind += 1;
								...
								.each(
										function(index, element) {
											let product_no = $(element)
													...
													.val();
											let sales = $(element).find(
													".individual_sales_input")
													.val();
